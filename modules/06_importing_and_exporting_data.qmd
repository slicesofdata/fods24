...
- [Data ImportR](https://r4ds.hadley.nz/data-import)
...
Although we will discuss data frames and [tibbles](https://r4ds.had.co.nz/tibbles.html) later, we can create data frames in `base R` using the `data.frame()` function. We would name a variable and then assign the observations/cases as elements of a vector using the combine function, `c()`. 
...
Using the `tribble()` function from the **{tibble}** library, we can also create a special type of data frame known as a [tibble](https://r4ds.had.co.nz/tibbles.html). For tibbles, the arrangement of data is similar to how the data would appear in a file -- variables as columns and cases as rows. 
