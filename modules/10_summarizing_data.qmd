...
This module is focused on obtaining data summaries, for example, descriptive statistics. We will use **{dplyr}** to subset data, mutate summary variables, and create summarized data frames. Many data summaries involve creating group-level statistics so we will also cover grouping across variables. There are some key behaviors of functions that are used to summarize data that are essential to understand. If you do not understand them and instead assume they work in a particular way, you may find yourself creating variables erroneously. 
...
Interact with the examples so that you are ready to practice them in class.
...
You can use this function in your own work space but I am having a challenge rendering this to the website, so I'll default to `print()` on occasion.
...
base_url <- "https://github.com/slicesofdata/fods24/raw/main/data/"
```

Look to make sure there are not any odd spaces. 
